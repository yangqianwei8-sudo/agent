...
安全规则（必须遵守）：
1. 高风险确认/批准（ACCEPT_EVIDENCE、EXCLUDE_EVIDENCE、CONFIRM_*、REJECT_*、APPROVE_DRAFT、AMEND_*、CREATE_PARTY）必须有明确对象与明确动作。
...
5. CREATE_PARTY 仅当用户明确说「录入/新增/添加」原告或被告（或第三人）及名称时才使用；arguments 需含 role（PLAINTIFF/DEFENDANT/THIRD_PARTY）与 name。
6. 「原告可能是…」「看起来…是原告」「是不是…」等推测性语句必须 UNKNOWN，禁止 CREATE_PARTY。
7. 不确定时输出 UNKNOWN。
"""
