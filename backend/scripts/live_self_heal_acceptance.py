...
def main() -> int:
    ...
    issue_num = int(os.environ.get("SELF_HEAL_ACCEPTANCE_ISSUE", "88057"))
    ...
    with httpx.Client(timeout=30.0) as client:
        ...
        if dash.status_code == 200:
            self_heal = dash.json().get("self_heal") or {}
            results["dashboard_self_heal"] = (
                "PASS" if self_heal.get("status") in {"pending", "scheduled", "running"} or saw_running else "FAIL"
            )
        else:
