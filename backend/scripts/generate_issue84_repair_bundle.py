#!/usr/bin/env python3
"""Generate Issue #84 repair SSOT bundle with fully inlined untruncated artifacts."""
...
EVIDENCE = ROOT / "docs" / "issue84_repair_evidence"
...
def _capture_header(repair_full: str, ts: datetime) -> str:
    return (
        f"# Issue #84 repair capture | commit={repair_full} | "
...
def _strip_capture_header(content: str) -> str:
    ...
    if lines and lines[0].startswith("# Issue #84 repair capture |"):
...
def build_bundle(ts: datetime, patch_base: str, repair_head: str, patch: str) -> str:
    ...
    header = f"""# Issue #84 Resubmit — Issue-Centered V2 (#80 repair)
...
| Production patch | {patch_lines} | `issue84_repair_production.patch` |
...
def build_evidence_md(
    ...
    return f"""# Issue #84 Repair Evidence — Issue-Centered V2 (#80)
...
| Repair patch | `issue84_repair_production.patch` ({patch_lines} lines, untruncated) |
...
def _write_bundle(
    ...
    evidence_path = EVIDENCE / "issue84_repair_evidence.md"
    ...
    evidence_path.write_text(
        build_evidence_md(
            ...
            patch_base,
            repair_head,
            "docs/issue84_repair_evidence/00_reviewer_bundle.md",
            patch,
        ),
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    patch_path = EVIDENCE / "issue84_repair_production.patch"
    patch_path.write_text(patch, encoding="utf-8")
    _write_bundle(ts, patch_base, repair_head, patch)
    # Tied to #73/#60 acceptance lineage, not #80 or #84.
