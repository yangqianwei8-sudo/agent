#!/usr/bin/env python3
"""Generate Issue #73 repair SSOT bundle with fully inlined untruncated artifacts."""
...
def _capture_header(repair_full: str, ts: datetime) -> str:
    return (
        f"# Issue #73 repair capture | commit={repair_full} | "
...
def _strip_capture_header(content: str) -> str:
    ...
    if lines and lines[0].startswith("# Issue #73 repair capture |"):
...
def build_bundle(ts: datetime, patch_base: str, repair_head: str, patch: str) -> str:
    ...
    header = f"""# Issue #73 Resubmit — Issue-Centered V2 (#60 repair)
...
| Production patch | {patch_lines} | `issue73_repair_production.patch` |
...
def build_evidence_md(
    ...
    return f"""# Issue #73 Repair Evidence — Issue-Centered V2 (#60)
...
| Repair patch | `issue73_repair_production.patch` ({patch_lines} lines, untruncated) |
...
def _write_bundle(
    ...
    evidence_path = EVIDENCE / "issue73_repair_evidence.md"
    ...
    evidence_path.write_text(
        build_evidence_md(
            ...
            patch_base,
            repair_head,
            "docs/issue73_repair_evidence/00_reviewer_bundle.md",
            patch,
        ),
        encoding="utf-8",
    )
