#!/usr/bin/env python3
"""Generate Issue #83 repair SSOT bundle with fully inlined untruncated artifacts."""
...
def _capture_header(repair_full: str, ts: datetime) -> str:
    return (
        f"# Issue #83 repair capture | commit={repair_full} | "
...
def _strip_capture_header(content: str) -> str:
    ...
    if lines and lines[0].startswith("# Issue #83 repair capture |"):
...
def _acceptance_lineage_section(ts_iso: str) -> str:
    return f"""## Acceptance lineage — tied to Issue #73 / Issue #60
...
This bundle regenerates the #83 repair SSOT with untruncated production artifacts and live PASS evidence at {ts_iso}."""
...
def build_bundle(ts: datetime, patch_base: str, repair_head: str, patch: str) -> str:
    ...
    header = f"""# Issue #83 Resubmit — Issue-Centered V2 (#80 repair, #73/#60 lineage)
...
| Production patch | {patch_lines} | `issue83_repair_production.patch` |
...
def build_evidence_md(
    ...
    return f"""# Issue #83 Repair Evidence — Issue-Centered V2 (#80, #73/#60 lineage)
...
| Repair patch | `issue83_repair_production.patch` ({patch_lines} lines, untruncated) |
...
def _write_bundle(
    ...
    evidence_path = EVIDENCE / "issue83_repair_evidence.md"
    ...
    evidence_path.write_text(
        build_evidence_md(
            ...
            patch_base,
            repair_head,
            "docs/issue83_repair_evidence/00_reviewer_bundle.md",
            patch,
        ),
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    patch_path = EVIDENCE / "issue83_repair_production.patch"
    patch_path.write_text(patch, encoding="utf-8")
    _write_bundle(ts, patch_base, repair_head, patch)
    # Tied to #73/#60 acceptance lineage, not #80 or #83.
    marker = ROOT / "autonomous_dev" / "acceptance_marker.txt"
    marker.write_text(
        f"worker-run issue=73 lineage=60 at={ts.isoformat()}\n", encoding="utf-8"
    )
